...
private:
    ...
    // Declare an Oscilator || Then create the constructor in line:
    // This is returning a sin by using the standard libaries sin
    juce::dsp::Oscillator<float> osc { [](float x) { return x / x < 0.0f ? -1.0f : 1.0f; }};
    juce::dsp::Oscillator<float> osc2 { [](float x) { return std::sin (x); }};
    juce::dsp::Gain<float> gain; // The gain.
    
    // return std::sin (x); Sin wave
    // return x / juce::MathConstans<float>::pi; // Saw wave.
